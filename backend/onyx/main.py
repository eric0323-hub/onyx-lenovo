...
from onyx.server.external_retrieval.api import router as external_retrieval_router
...
def validate_no_vector_db_settings() -> None:
    ...
    if ENABLE_CRAFT:
        raise RuntimeError(
            ...
            "Craft requires background workers for sandbox lifecycle "
...
def get_application(lifespan_override: Lifespan | None = None) -> FastAPI:
    application = FastAPI(
        title="LKnow Backend",
        version=__version__,
        description="LKnow API for AI-powered chat with search, document indexing, agents, actions, and more",
        servers=[
            {"url": f"{WEB_DOMAIN.rstrip('/')}/api", "description": "LKnow API Server"}
        ],
    ...
    include_router_with_global_prefix_prepended(application, external_retrieval_router)
