...
def _check_bot_config_api_access() -> None:
    ...
    - On Cloud (managed by LKnow)
    - When DISCORD_BOT_TOKEN env var is set (managed via env)
    """
    if AUTH_TYPE == AuthType.CLOUD:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Discord bot configuration is managed by LKnow on Cloud.",
        )
...
def delete_service_api_key_endpoint(
    ...
    Discord bot to authenticate with the LKnow API. The key is also automatically
    deleted when:
